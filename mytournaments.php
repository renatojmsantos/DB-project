      		...
      		if ($_SESSION['admin'] == TRUE)  {
			    ...
			    if($num_of_results>0){
					...
					while ($row = $query->fetch_assoc()){
			            ...
			            <td><input type="submit" value="Apagar" onclick="apagaTorneio(this)"></td> 
			            </tr> ';
					}
					echo "</table></center>";
			    }else{
      			...
      			//echo"nao admin";
      			$query = $connection->query("SELECT tournament.name, tournament.descricao, tournament.datainicio, tournament.datafim FROM tournament, team_tatica, role_in_team,user WHERE tournament.name = team_tatica.tournament_name AND role_in_team.team_id = team_tatica.id AND role_in_team.user_cc = user.cc AND user.username = '{$_SESSION['user_login']}'
");
		...
		<?php if ($_SESSION['admin'] == TRUE) : ?>
			<form action="criarTorneio.php">
				<button type="submit" style = "font-size:20px;border-radius:8px">Criar Torneio</button>
		    </form>
		<?php endif; ?>
	</div>
</body>
